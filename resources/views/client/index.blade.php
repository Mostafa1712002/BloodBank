        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    {{ Form::input('date', 'd_o_b', date('Y-m-d',  00-00-00 ), array('class' => 'form-control')) }}
                </div>

                <div class="form-group">
                    {!! Form::label("client_last_donation_date","تصفية حسب أخر تاريخ تبرع") !!}
                    {{ Form::input('date', 'last_donation_date', date('Y-m-d',  00-00-00 ), array('class' => 'form-control')) }}
                </div>
                ...
                <table class="table table-hover ">
                    <thead class="thead-dark">
                        <tr>
                            ...
                            @if (auth()->user()->can("active-client") || auth()->user()->can("de-active-client"))
                    ...
                    <tbody>
                        ...
                        <tr data-row="#row{{ $record->id }}" id="row{{ $record->id  }}">
                            ...
                            <td scope="row" class="text-center">
                                ...
                                <button class="btn  btn-success btn-sm"> <span class="font-bold"> تفعيل</span> </button>
                                @endif
                                {{ Form::close() }}
                                @else
                                {{ Form::open([
                            ...
                            @if (auth()->user()->can("destroy-client"))
                            <td scope="row" class="text-center">
                                <div class="btn btn-danger destroy btn-sm" data-route="{{ route("client.destroy",$record->id) }}" data-token="{{ csrf_token() }}">
                        ...
                        , action: function() {
                            $.ajax({
                                ...
                                , success: function(data) {
                                    if (data.status === 1) {
                                        $($row).remove();
                                        Swal.fire("تم حذف العميل بنجاح", " ", "success");
                                    }
                                }
                                , error: function() {
